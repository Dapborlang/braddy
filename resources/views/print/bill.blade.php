...
<body>
        @php 
            ...
            $figure->setAttribute(\NumberFormatter::MAX_FRACTION_DIGITS, 0);
        @endphp
   	<table border="1" width="100%" id="table2">
	   			...
	   			<th colspan="7">
	   				<table border="0" width="60%">
	   					...
	   					<tr>
	   						...
	   						<td style="text-align: right;><span style="font-size: 12px;">Bill Date: {{$bill->bill_date}}</span></td>
	   					</tr>
	   					<tr>
	   						<td style="text-align: left;"><span style="font-size: 12px;">Order No: {{$bill->order_no}}</span></td>
	   						<td style="text-align: right;><span style="font-size: 12px;">Order Date: {{$bill->order_date}}</span></td>
	   					</tr>
	   				</table>
	   			</th>
	   		</tr>
        ...
        @if($bill->tax_type=="I")
            @foreach($bill->BillDetail as $item)
                <tr>
                    ...
                    <td style="text-align: right;">{{($item->rate * $item->quantity)-(round($item->rate - ($item->rate*$item->gst)/(100+$item->gst), 2)*$item->quantity)}}</td>
                    <td style="text-align: right;">{{$item->rate * $item->quantity}} @php $total[]=$item->rate * $item->quantity @endphp</td>
                </tr>
            @endforeach
        @else
            @foreach($bill->BillDetail as $item)
        ...
        @endif
  			<tr>
  				...
  				<td style="text-align: right; font-size: 15px; font-weight: bold;"></td>
  				<td style="text-align: right; font-size: 15px; font-weight: bold;">{{array_sum($total)}}</td>
  			</tr>
		</tbody>
		<tfoot>
		    <tr>
		    ...
		    <tr>
		    	<td colspan="5" style="text-align: center; font-size: 15px; font-weight: bold;">(Rupees {{$words->format(array_sum($total))}}) only.</td>
		    </tr>
		</tfoot>
	</table>
	</div>
